...
class SubmissionApi extends GoldApi {
	...
	protected function quickSearch(string $search): Filter {
		return Filter::where(Submission::title()->instring($search))
		             ->or(Submission::category()->instring($search))
		             ->or(Submission::id($search))
			;
	}
	...
	public function addViews(): void {
		...
		$this->addView(new GoldView('draft', 'draft', fn() => Filter::where(Submission::status(Submission::status__draft))));
	...
	public function addSortings(): void {
		...
		$this->addSorting(new GoldSorting('category', 'category', fn(bool $asc)=> $asc ? [[Submission::title(), "asc"]] : [[Submission::title(), "desc"]]));
	}
